...
import type {
    ...
    FunilConversaoRow, FeedbackStats, LeadPerdidoRetorno
} from './types';
...
// ===== COMERCIAL - NOVAS FUNÇÕES (Phase 1) =====

// Task 1.1: Hook useLeadsPerdidos
export function useLeadsPerdidos(filtros?: { responsavel_id?: string; motivo?: string }) {
    const [vencidos, setVencidos] = useState<Deal[]>([]);
    const [proximos7d, setProximos7d] = useState<Deal[]>([]);
    const [proximasemanas, setProximasemanas] = useState<Deal[]>([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);

    const fetchLeadsPerdidos = async () => {
        setLoading(true);
        try {
            const today = new Date();
            const in7days = new Date(today.getTime() + 7 * 24 * 60 * 60 * 1000);

            let query = supabase.from('deals').select('*').eq('stage', 'perdido');

            if (filtros?.responsavel_id) {
                query = query.eq('assignee', filtros.responsavel_id);
            }
            if (filtros?.motivo) {
                query = query.ilike('motivo_perda', `%${filtros.motivo}%`);
            }

            const { data: deals, error: err } = await query;
            if (err) throw err;

            // Separar por categoria de retorno
            const vencidosArr: Deal[] = [];
            const proximos7dArr: Deal[] = [];
            const proximasSemanaArr: Deal[] = [];

            (deals || []).forEach(deal => {
                if (!deal.data_retorno) return;
                const retorno = new Date(deal.data_retorno);
                if (retorno <= today) {
                    vencidosArr.push(deal);
                } else if (retorno <= in7days) {
                    proximos7dArr.push(deal);
                } else {
                    proximasSemanaArr.push(deal);
                }
            });

            setVencidos(vencidosArr.sort((a, b) => new Date(a.data_retorno!).getTime() - new Date(b.data_retorno!).getTime()));
            setProximos7d(proximos7dArr.sort((a, b) => new Date(a.data_retorno!).getTime() - new Date(b.data_retorno!).getTime()));
            setProximasemanas(proximasSemanaArr.sort((a, b) => new Date(a.data_retorno!).getTime() - new Date(b.data_retorno!).getTime()));
            setError(null);
        } catch (err) {
            setError(err instanceof Error ? err.message : 'Erro desconhecido');
            console.error('Error fetching leads perdidos:', err);
        }
        setLoading(false);
    };
    ...
    useEffect(() => {
        fetchLeadsPerdidos();
    }, [filtros?.responsavel_id, filtros?.motivo]);

    return { vencidos, proximos7d, proximasemanas, loading, error, refetch: fetchLeadsPerdidos };
}

// Task 1.2: Hook useConversaoFunil
export function useConversaoFunil(dataInicio: string, dataFim: string) {
    const [data, setData] = useState<FunilConversaoRow[]>([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    ...
    const fetchConversao = async () => {
        ...
        try {
            ...
            // Agrupar por stage
            const stages = ['prospeccao', 'diagnostico', 'negociacao', 'fechado', 'perdido'];
            const result: FunilConversaoRow[] = [];

            stages.forEach((stage, idx) => {
                const stageDeals = (deals || []).filter(d => d.stage === stage);
                const quantidade = stageDeals.length;
                const valor_total = stageDeals.reduce((sum, d) => sum + (d.value || 0), 0);

                // Taxa de conversão = próximo stage / este stage
                let taxa_conversao = 0;
                if (quantidade > 0 && idx < stages.length - 1) {
                    const nextStageName = stages[idx + 1];
                    const nextStageDeals = (deals || []).filter(d => d.stage === nextStageName);
                    taxa_conversao = Math.round((nextStageDeals.length / quantidade) * 100);
                }

                // Tempo médio na etapa
                const dias_medio = stageDeals.length > 0
                    ? Math.round(
                        stageDeals.reduce((sum, d) => {
                            const entrada = new Date(d.data_entrada_etapa || d.created_at).getTime();
                            const agora = new Date().getTime();
                            return sum + (agora - entrada) / (1000 * 60 * 60 * 24);
                        }, 0) / stageDeals.length
                    )
                    : 0;

                // Responsáveis únicos
                const responsaveis = [...new Set(stageDeals.map(d => d.assignee))].filter(Boolean);
            ...
            setError(err instanceof Error ? err.message : 'Erro desconhecido');
            console.error('Error fetching conversao funil:', err);
        }
        setLoading(false);
    };
    ...
    return { data, loading, error, refetch: fetchConversao };
}

// Task 1.3: Hook useFeedbackStats
export function useFeedbackStats(clientId: string) {
    const [stats, setStats] = useState<FeedbackStats>({ elogios: 0, sugestoes: 0, reclamacoes: 0 });
    const [loading, setLoading] = useState(true);
    ...
    useEffect(() => {
        const fetchStats = async () => {
            ...
            if (!error && feedbacks) {
                setStats({
                    elogios: feedbacks.filter(f => f.type === 'Elogio').length,
                    sugestoes: feedbacks.filter(f => f.type === 'Sugestão').length,
                    reclamacoes: feedbacks.filter(f => f.type === 'Reclamação').length
                });
            }
            setLoading(false);
        };
...
// Task 1.4: Função sincronizarClienteCRMpDiretorio
export async function sincronizarClienteCRMpDiretorio(clienteCRMId: string) {
    try {
        // Buscar cliente do CRM
        const { data: clienteCRM, error: err1 } = await supabase
            .from('clientes_crm')
            .select('*')
            .eq('id', clienteCRMId)
            .single();
        ...
        if (err1) throw err1;
        if (!clienteCRM) throw new Error('Cliente CRM não encontrado');

        // Mapear campos comuns
        const dirClient = {
            nome: clienteCRM.name,
            segmento: clienteCRM.segment,
            email: clienteCRM.contact_email,
            site: clienteCRM.website,
            status: clienteCRM.status?.toLowerCase() === 'ativo' ? 'ativo' : 'inativo',
            cliente_crm_id: clienteCRMId,
            sincronizado_em: new Date().toISOString()
        };

        // Buscar se já existe no diretório
        const { data: existing } = await supabase
            .from('diretorio_clientes')
            .select('id')
            .eq('cliente_crm_id', clienteCRMId)
            .single();
        ...
        return { sucesso: true, cliente_diretorio_id: result.id };
    } catch (err) {
...
// Task 1.5: Função updateDealRetorno
export async function updateDealRetorno(dealId: string, data: {
    ...
    try {
        ...
        // Criar notificação para todos com /comercial
        const { data: usuarios } = await supabase
            .from('usuarios')
            .select('id')
            .contains('modulos_acesso', ['/comercial']);

        if (usuarios) {
            const notificacoes = usuarios.map(u => ({
                usuario_id: u.id,
                tipo: 'lead_retorno_vencido',
                titulo: 'Lead para Retomar',
                mensagem: `Lead movido para retorno em ${new Date(data.data_retorno).toLocaleDateString('pt-BR')}`,
                lida: false,
        ...
        console.error('Error updating deal retorno:', err);
        throw err;
    }
}
