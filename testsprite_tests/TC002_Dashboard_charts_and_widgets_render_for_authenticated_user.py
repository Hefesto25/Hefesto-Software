...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        assert '/' in frame.url
        await expect(frame.locator('text=Bar').first).to_be_visible(timeout=3000)
        await expect(frame.locator('text=Line').first).to_be_visible(timeout=3000)
        await expect(frame.locator('text=Pie').first).to_be_visible(timeout=3000)
