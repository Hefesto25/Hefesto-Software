...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Financeiro' sidebar link (index 624) to open the Finance section so the DRE tab can be located.
        frame = context.pages[-1]
        # Click element
        ...
        frame = context.pages[-1]
        await expect(frame.locator('text=DRE').first).to_be_visible(timeout=3000)
        await expect(frame.locator('xpath=//*[normalize-space(text())="DRE"]/following::svg[1]').first).to_be_visible(timeout=3000)
